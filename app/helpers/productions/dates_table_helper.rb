module Productions::DatesTableHelper
  ...
  def dates_table p
    if dates_single_date_tbc?(p)
      ...
      return bookend_tags('table', dates_tbc_row, { class: 'table dates-tbc-table' })
    end
    ...
    if single_date?(p)
      ...
      return bookend_tags('table', compile_rows({ row_data: row_data }), { class: 'table dates-table' })
    end
    ...
    bookend_tags('table', compile_rows({ row_data: dates.compact }), { class: 'table dates-table' })
  end

  def get_dates_markup p
    dates_note = p.dates_note.present? ? "<p class='note-text emphasis-text'>#{p.dates_note}</p>" : ''
    dates_content = "<div class='content-label'>Dates</div>#{dates_table(p)}#{dates_note}"
    @dates = bookend_tags('div', dates_content, { id: 'dates', class: 'content-wrapper' }).html_safe
  end
